...
import 'package:fpdart/fpdart.dart' 
  show Left, None, Right, Some, Task, TaskEither, TaskOption, identity;
...
Task<AppModel> update(AppModel model, Msg msg) => switch (msg) {
  PostCreated() => Task.of(model.copyWith(
    createPostTitle: '',
  )),

  CreatePostTitleChanged(:final title) => Task.of(model.copyWith(
    createPostTitle: title,
  )),

  LoadWorkspace(:final user) => Task.Do(
    ($) async {
      final workspace = await fetchWorkspace(user).run();

      return workspace.match(
        () => model, 
        (ws) => model.copyWith(currentWorkspace: Some(ws)),
      );
    }
  ),
  ...
  LoginSuccess(:final user, :final online) => Task.Do(
    ($) async {
      ...
      return online 
        ? await $(update(newModel, LoadWorkspace(user)))
        : newModel;
    }
  ),
...
Widget view(AppModel model, void Function(Msg) dispatch) => MaterialApp(
  ...
  home: Builder(builder: (context) {
    ...
    return HomePage(
      ...
      onPostCreated: () => dispatch(PostCreated()),
      onCreatePostTitleChanged: (title) => dispatch(CreatePostTitleChanged(title)),
    );
  }),
);
...
TaskOption<Workspace> fetchWorkspace(LoggedUser user) => TaskOption.Do(
  ($) async {
    final prefs = await SharedPreferences.getInstance();
    final workspace = getJsonFromPrefs(
      prefs: prefs, 
      key: 'frago_workspace',
    ...
    return workspace.match(
      () => $(TaskOption.Do(
        ($) async {
          final checkRemoteWorkspace = await $(
            TaskOption.tryCatch(() async => await http.get(
              Uri.parse(
                'https://api.github.com/repos/${user.name}/${user.name}.github.io'
              ),
            )),
          );

          return switch (checkRemoteWorkspace.statusCode) {
            200 => $(TaskOption.Do(
              ($) async {
                final defaultWorkspace = Workspace.initial();
                await prefs.setString(
                  'frago_workspace', 
                  jsonEncode(defaultWorkspace.toJson()),
                );
